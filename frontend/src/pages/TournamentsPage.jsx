...
function toCard(t) {
  ...
  // Backend: PUBLISHED, COMPLETED, CANCELED
  // UI: open, live, completed
  let status = 'open';
  if (t.status === 'COMPLETED') status = 'completed';
  else if (t.status === 'PUBLISHED') status = start <= now ? 'live' : 'open';
  else if (t.status === 'CANCELED') return null;
  ...
  return {
    id: t.id,
    game: t.title || t.game, // show title as the card heading
    status,
    ...
    players: 0, // we'll wire this later when join is implemented
...
const TournamentsPage = () => {
  ...
  const filters = [
    ...
    { id: 'upcoming', label: t('tournaments.upcoming') }, // treat as open
  ...
  const filteredTournaments = useMemo(() => {
    ...
    return items.filter((tour) => {
      const matchesFilter =
        selectedFilter === 'all' ||
        tour.status === selectedFilter ||
        (selectedFilter === 'upcoming' && tour.status === 'open'); // alias
